$(function(){
	jQuery(document).ready(function() {
	    jQuery('.scrollContent').jcarousel({
	        vertical: true,
			animation: "slow",
			wrap: 'circular',
	        scroll: 1
...
$(document).ready(function(){
            ...
            $('#balance_phone').live('click',function(){
                $.blockUI({ 
                    message: $('#payment_step1'),
                    css:{
                        cursor: 'default',
                        top: '15%', 
                        left: '35%',
            ...
            $('#set_cab, #set_cab span').live('click',function() { 
                $.blockUI({ 
                    message: $('#settings_cab'),
                    css:{
                        cursor: 'default', 
                        top: '1%',
                        left: '29%',
            ...
            $('.close, .blockOverlay #close_fini').live('click',function(){
                $.unblockUI();
            });
            
            $('#curency').live('click',function(){
                lin = $('#block-cur')
                op = lin.css('opacity');
                dis = lin.css('display');
                if(op == "0" || dis == "none")
                ...
                self = this;
                $('.c-content a em').live('click',function(){
                    _in = $(self).attr('type');
                    _out = $(this).attr('type');
                    _self = this;
                    $.ajax({
                        type: 'POST',
                        url: '/ajax/convert_currency',
                        data:{
                            'in': _in,
                            out: _out
                        },
                        ...
                        success: function(data){
                            ...
                            .attr('type',data.in_)
                            .text(data.in_);
                            $('#balance_phone').text(data.newval);
                            lin.animate({
                                opacity: 0
                            },150);
                        }
            ...
            $('#form_lang .lng_img').live('click',function(){
                self = this;
                if($('select#lang').css('display') == 'block')
                {
                    ...
                    .live('change',function(){
                        $(self)
                        .children('img')
                        .attr('src','/sipcms/images/lang-ico-us.png');
                        $('select#lang').css({
                            display: 'none'
                        });
                        $(self).css({
                        ...
                        return false;
                        //TODO:Send AJAX change language
                    });
                    $(this).css({
                        left: '-30px'
                    });
